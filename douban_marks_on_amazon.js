...
// 2015-01-29   update
// 2016-01-03	update douban marks on kindle pages
// ==/UserScript==
...
!function() {
	...
	var xht2 = function() {
		GM_xmlhttpRequest({
			...
			onload : function(res) {
				...
				// var emp = $('.buying')[pnum];
				var emp = $('#productGuarantee_feature_div')[0];
				if (!emp) {
					emp = $('#tafContainerDiv')[0];
				}
				var pos;
				...
				htmlstr.innerHTML = "&nbsp|&nbsp<span style='color:#0C7823; font-weight:700;'>豆瓣</span>评分:<span style='background-image:url(http://img3.douban.com/pics/all_bigstars.gif); background-repeat:no-repeat; background-position:0 "
						+ pos
						+ "px; width:75px;'>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</span>";

				if (numRaters < 10)
					htmlstr.innerHTML += "<span>&nbsp少于10人评价&nbsp<a href='"
							+ link + "'>链接</a></span>";
				else
					htmlstr.innerHTML += "<span>" + average + "&nbsp("
							+ numRaters + "人评价)&nbsp<a href='" + link
							+ "'>链接</a></span>";
				console.log(htmlstr);
	...
	if (isbn === '') {
		...
		for (var i = 0; i <= 2; i++) {
			olink = olinks[i];
			if (olink) {
				if (olink.textContent == "平装" || olink.textContent == "精装") {
					olink = olink.href;
					break;
					// olink = olink.split("/")[5];
